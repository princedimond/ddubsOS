...
# Control whether to print the summary on exit (suppressed for --help/--dry-run)
PRINT_SUMMARY=true
...
# Initialize LOG_FILE to avoid unbound variable issues under set -u; it will be set after directories are created
LOG_FILE=""
...
cleanup() {
    local exit_code=$?
    if [ "${PRINT_SUMMARY:-true}" = true ]; then
        print_summary
    fi
    exit $exit_code
}
...
# Check if script is being sourced or executed (robust under set -u)
