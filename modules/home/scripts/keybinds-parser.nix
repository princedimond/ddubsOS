...
pkgs.writeShellScriptBin "keybinds-parser" ''
    ...
    case "$MODE" in
      hyprland)
        # First, try to parse ddubsos bindd entries from Nix config for richer descriptions
        BIND_NIX="$HOME/ddubsos/modules/home/hyprland/binds.nix"
  ...
  BEGIN {
      ...
      sway)
        ...
        if [[ ! -f "$SWAY_CONFIG" ]]; then
          SWAY_CONFIG="$HOME/ddubsos/modules/home/gui/sway/files/config_cuerdos"
          if [[ ! -f "$SWAY_CONFIG" ]]; then
