...
pkgs.writeShellScriptBin "qs-keybinds" ''
  ...
  Options:
    -m MODE  Mode to display (hyprland|niri|bspwm|i3|sway|dwm|emacs|kitty|wezterm|yazi|cheatsheets) (default: hyprland)
    -h       Show this help
  EOF
    }
    ...
    # Validate mode
    if [[ "$MODE" != "hyprland" && "$MODE" != "niri" && "$MODE" != "bspwm" && "$MODE" != "i3" && "$MODE" != "sway" && "$MODE" != "dwm" && "$MODE" != "emacs" && "$MODE" != "kitty" && "$MODE" != "wezterm" && "$MODE" != "yazi" && "$MODE" != "cheatsheets" ]]; then
      echo "Error: Invalid mode '$MODE'. Use 'hyprland', 'niri', 'bspwm', 'dwm', 'emacs', 'kitty', 'wezterm', 'yazi', or 'cheatsheets'" >&2
      exit 1
    fi
    ...
    HAS_NIRI=0; { [ -f "$HOME/.config/niri/config.kdl" ] || command -v niri >/dev/null 2>&1; } && HAS_NIRI=1 || true
    ...
    HAS_BSPWM=0; { [ -f "$HOME/.config/bspwm/bspwmrc" ] || command -v bspwm >/dev/null 2>&1; } && HAS_BSPWM=1 || true
    HAS_I3=0; { [ -f "$HOME/.config/i3/config" ] || [ -f "$HOME/.config/i3/sxhkd/sxhkdrc" ] || command -v i3 >/dev/null 2>&1 || command -v i3-msg >/dev/null 2>&1 || echo "''${XDG_CURRENT_DESKTOP:-}" | ${pkgs.gnugrep}/bin/grep -qi i3 || echo "''${DESKTOP_SESSION:-}" | ${pkgs.gnugrep}/bin/grep -qi i3; } && HAS_I3=1 || true
    HAS_SWAY=0; { [ -f "$HOME/.config/sway/config" ] || command -v sway >/dev/null 2>&1 || command -v swaymsg >/dev/null 2>&1 || echo "''${XDG_CURRENT_DESKTOP:-}" | ${pkgs.gnugrep}/bin/grep -qi sway || echo "''${DESKTOP_SESSION:-}" | ${pkgs.gnugrep}/bin/grep -qi sway; } && HAS_SWAY=1 || true
    HAS_DWM=0; command -v dwm >/dev/null 2>&1 && HAS_DWM=1 || true

    # Allow host-stateful overrides via env (set to 0/1). These can be wired from hosts/<hostname>/variables.nix
    [ -n "''${QS_HAS_NIRI:-}" ] && HAS_NIRI="''${QS_HAS_NIRI}"
    [ -n "''${QS_HAS_HYPR:-}" ] && HAS_HYPR="''${QS_HAS_HYPR}"
    [ -n "''${QS_HAS_BSPWM:-}" ] && HAS_BSPWM="''${QS_HAS_BSPWM}"
    [ -n "''${QS_HAS_I3:-}" ] && HAS_I3="''${QS_HAS_I3}"
    [ -n "''${QS_HAS_SWAY:-}" ] && HAS_SWAY="''${QS_HAS_SWAY}"
    [ -n "''${QS_HAS_DWM:-}" ] && HAS_DWM="''${QS_HAS_DWM}"

    MODE_AVAILABLE=1

    if [ "$MODE" = "cheatsheets" ]; then
      # For cheatsheets mode, generate file listing and categories
      ${pkgs.callPackage ./cheatsheets-parser.nix {}}/bin/cheatsheets-parser files emacs en > "$json"
      ${pkgs.callPackage ./cheatsheets-parser.nix {}}/bin/cheatsheets-parser categories > "$tmpdir/categories.json"
      # Generate content for all categories and languages
      for category in emacs hyprland kitty wezterm yazi nixos; do
        if [ -d "$HOME/ddubsos/cheatsheets/$category" ]; then
          ${pkgs.callPackage ./cheatsheets-parser.nix {}}/bin/cheatsheets-parser files "$category" en > "$tmpdir/files_''${category}_en.json"
          ${pkgs.callPackage ./cheatsheets-parser.nix {}}/bin/cheatsheets-parser files "$category" es > "$tmpdir/files_''${category}_es.json"
        fi
      done
    else
      ...
      # For Kitty, WezTerm, and Yazi modes, also generate submode data when available
      if [ "$MODE_AVAILABLE" -eq 1 ] && { [ "$MODE" = "kitty" ] || [ "$MODE" = "wezterm" ] || [ "$MODE" = "yazi" ]; }; then
    ...
    case "$MODE" in
      ...
      niri) WINDOW_TITLE="Niri Keybinds" ;;
      bspwm) WINDOW_TITLE="BSPWM Keybinds" ;;
      i3) WINDOW_TITLE="i3 Keybinds" ;;
      sway) WINDOW_TITLE="Sway Keybinds" ;;
      dwm) WINDOW_TITLE="DWM Keybinds" ;;
      ...
      cheatsheets) WINDOW_TITLE="Cheatsheets Viewer" ;;
    esac
  ...
  Window {
    id: win
    visible: true
    width: selectedMode === "cheatsheets" ? 1400 : 1050
    height: selectedMode === "cheatsheets" ? 900 : 700
    ...
    property bool hasNiri: HAS_NIRI_PLACEHOLDER
    property bool hasBspwm: HAS_BSPWM_PLACEHOLDER
    property bool hasI3: HAS_I3_PLACEHOLDER
    property bool hasSway: HAS_SWAY_PLACEHOLDER
    property bool hasDwm: HAS_DWM_PLACEHOLDER

    // Cheatsheets mode properties
    property string selectedCheatCategory: "emacs"
    property string selectedLanguage: "en"
    property var cheatsheetFiles: []
    property string selectedCheatFile: ""
    property string cheatsheetContent: ""
    property string cheatSearchQuery: ""
    ...
    // Load cheatsheet files for specific category and language
    function loadCheatsheetFiles(category, language) {
      console.log("Loading cheatsheet files for:", category, language);

      var fileName = "files_" + category + "_" + language + ".json";
      var filePath = jsonDataFile.replace("keybinds.json", fileName);
          ...
          if (xhr.status === 200 || xhr.status === 0) {
            try {
              ...
              if (result.length > 0) {
                ...
                if (!win.selectedCheatFile) {
                  win.selectedCheatFile = result[0].filename;
                  win.loadCheatsheetContent(category, result[0].filename);
                }
              }
            } catch (e) {
              console.error("Failed to parse cheatsheet files JSON:", e);
              win.cheatsheetFiles = [];
            }
          }
        }
    ...
    // Load content of a specific cheatsheet file
    function loadCheatsheetContent(category, filename) {
      console.log("Loading cheatsheet content:", category, filename);

      const filePath = "/home/dwilliams/ddubsos/cheatsheets/" + category + "/" + filename;

      const xhr = new XMLHttpRequest();
      xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
          if (xhr.status === 200 || xhr.status === 0) {
            win.cheatsheetContent = xhr.responseText;
            win.selectedCheatFile = filename;
          } else {
            win.cheatsheetContent = "Error loading file: " + filename;
          }
        }
      };
      xhr.open("GET", "file://" + filePath);
    ...
    function getCategories() {
      // For Kitty, WezTerm, and Yazi modes, show app-specific sub-modes instead of regular categories
    ...
    Component.onCompleted: {
      ...
      if (selectedMode === "cheatsheets") {
        loadCheatsheetFiles(selectedCheatCategory, selectedLanguage);
      } else {
        loadKeybindsData();
      }
    }
    ...
    Rectangle {
      ...
      Column {
        ...
        Column {
          ...
          // Top row: Window managers (Hyprland, Niri) + search bar
          Row {
            id: topRow
            width: parent.width
            spacing: 16
            ...
            Row {
              ...
              Button {
                ...
                visible: hasNiri
                background: Rectangle {
                  color: selectedMode === "niri" ? "#66ccff" : "#444444"
                  radius: 6
                  border.width: 1
                  border.color: "#66ccff"
                }
                contentItem: Text {
                  text: parent.text
                  color: selectedMode === "niri" ? "#000000" : "#ffffff"
                  font.pixelSize: 14
                  horizontalAlignment: Text.AlignHCenter
                  verticalAlignment: Text.AlignVCenter
                }
                onClicked: { console.log("MODE:niri"); Qt.quit(); }
              }
              ...
              Button {
                ...
                visible: hasBspwm
                background: Rectangle {
                  color: selectedMode === "bspwm" ? "#66ccff" : "#444444"
                  radius: 6
                  border.width: 1
                  border.color: "#66ccff"
                }
                contentItem: Text {
                  text: parent.text
                  color: selectedMode === "bspwm" ? "#000000" : "#ffffff"
                  font.pixelSize: 14
                  horizontalAlignment: Text.AlignHCenter
                  verticalAlignment: Text.AlignVCenter
                }
                onClicked: { console.log("MODE:bspwm"); Qt.quit(); }
              }
              ...
              Button {
                ...
                visible: hasI3
                background: Rectangle {
                  color: selectedMode === "i3" ? "#66ccff" : "#444444"
                  radius: 6
                  border.width: 1
                  border.color: "#66ccff"
                }
                contentItem: Text {
                  text: parent.text
                  color: selectedMode === "i3" ? "#000000" : "#ffffff"
                  font.pixelSize: 14
                  horizontalAlignment: Text.AlignHCenter
                  verticalAlignment: Text.AlignVCenter
                }
                onClicked: { console.log("MODE:i3"); Qt.quit(); }
              }
              ...
              Button {
                ...
                visible: hasSway
                background: Rectangle {
                  color: selectedMode === "sway" ? "#66ccff" : "#444444"
                  radius: 6
                  border.width: 1
                  border.color: "#66ccff"
                }
                contentItem: Text {
                  text: parent.text
                  color: selectedMode === "sway" ? "#000000" : "#ffffff"
                  font.pixelSize: 14
                  horizontalAlignment: Text.AlignHCenter
                  verticalAlignment: Text.AlignVCenter
                }
                onClicked: { console.log("MODE:sway"); Qt.quit(); }
              }
              ...
              Button {
                ...
                visible: hasDwm
                background: Rectangle {
                  color: selectedMode === "dwm" ? "#66ccff" : "#444444"
                  radius: 6
                  border.width: 1
                  border.color: "#66ccff"
                }
                contentItem: Text {
                  text: parent.text
                  color: selectedMode === "dwm" ? "#000000" : "#ffffff"
                  font.pixelSize: 14
                  horizontalAlignment: Text.AlignHCenter
                  verticalAlignment: Text.AlignVCenter
                }
                onClicked: { console.log("MODE:dwm"); Qt.quit(); }
              }
            }
            ...
            Rectangle {
              id: searchBar
              width: parent.width - modeButtonsRowTop.width - 40  // Account for mode buttons width + spacing
              height: 36
              radius: 8
              border.width: 1
              border.color: "#66ccff"
          ...
          Row {
            ...
            Button {
              ...
              height: 36
              background: Rectangle {
                color: selectedMode === "cheatsheets" ? "#66ccff" : "#444444"
                radius: 6
                border.width: 1
                border.color: "#66ccff"
              }
              contentItem: Text {
                text: parent.text
                color: selectedMode === "cheatsheets" ? "#000000" : "#ffffff"
                font.pixelSize: 14
                horizontalAlignment: Text.AlignHCenter
                verticalAlignment: Text.AlignVCenter
              }
              onClicked: { console.log("MODE:cheatsheets"); Qt.quit(); }
            }
          }
        }
        ...
        Item {
          ...
          // Original keybinds interface (for non-cheatsheets modes)
          Column {
            anchors.fill: parent
            spacing: 12
            visible: selectedMode !== "cheatsheets"
              ...
              Repeater {
                ...
                Button {
                  ...
                  onClicked: {
                    ...
                    // For Kitty, WezTerm, and Yazi modes, reload data with submode instead of just filtering
                    if ((win.selectedMode === "kitty" || win.selectedMode === "wezterm" || win.selectedMode === "yazi") && modelData !== "all") {
  ...
  QML_TEMPLATE
    ...
    ${pkgs.gnused}/bin/sed -i "s/HAS_NIRI_PLACEHOLDER/$(emit_bool \"$HAS_NIRI\")/g" "$qml"
    ${pkgs.gnused}/bin/sed -i "s/HAS_BSPWM_PLACEHOLDER/$(emit_bool \"$HAS_BSPWM\")/g" "$qml"
    ${pkgs.gnused}/bin/sed -i "s/HAS_I3_PLACEHOLDER/$(emit_bool \"$HAS_I3\")/g" "$qml"
    ${pkgs.gnused}/bin/sed -i "s/HAS_SWAY_PLACEHOLDER/$(emit_bool \"$HAS_SWAY\")/g" "$qml"
    ${pkgs.gnused}/bin/sed -i "s/HAS_DWM_PLACEHOLDER/$(emit_bool \"$HAS_DWM\")/g" "$qml"
