...
Options:
  -m MODE  Mode to display (hyprland|niri|bspwm|dwm|emacs|kitty|wezterm|yazi|cheatsheets) (default: hyprland)
  -h       Show this help
EOF
  }
  ...
  # Validate mode
  if [[ "$MODE" != "hyprland" && "$MODE" != "niri" && "$MODE" != "bspwm" && "$MODE" != "dwm" && "$MODE" != "emacs" && "$MODE" != "kitty" && "$MODE" != "wezterm" && "$MODE" != "yazi" && "$MODE" != "cheatsheets" ]]; then
...
QML_TEMPLATE
  ...
  ${pkgs.gnused}/bin/sed -i "s/MODE_AVAILABLE_PLACEHOLDER/$([ "$MODE_AVAILABLE" -eq 1 ] && echo true || echo false)/g" "$qml"
  ${pkgs.gnused}/bin/sed -i "s/HAS_NIRI_PLACEHOLDER/$([ "$HAS_NIRI" -eq 1 ] && echo true || echo false)/g" "$qml"
  ${pkgs.gnused}/bin/sed -i "s/HAS_BSPWM_PLACEHOLDER/$([ "$HAS_BSPWM" -eq 1 ] && echo true || echo false)/g" "$qml"
  ${pkgs.gnused}/bin/sed -i "s/HAS_DWM_PLACEHOLDER/$([ "$HAS_DWM" -eq 1 ] && echo true || echo false)/g" "$qml"
