{
  pkgs,
  profile,
  backupFiles ? [".config/mimeapps.list.backup" ".config/nocatalia/settings.json.backup"],
  ...
}: let
...
in
  pkgs.writeShellScriptBin "zcli" ''
    ...
    print_help() {
      ...
      echo "  update-warp     - Update vendored Warp (versions.json + SRI); optional --commit."
      ...
      echo ""
      echo "Doom Emacs:"
      echo "  doom upgrade    - Upgrade Doom Emacs core and packages (stops/starts Emacs daemon)."
      echo "  doom status     - Show Doom presence and Emacs daemon status."
      echo "  doom start      - Start the Emacs user daemon."
      echo "  doom stop       - Stop the Emacs user daemon."
      echo "  doom restart    - Restart the Emacs user daemon."
      echo "  doom logs       - Show Emacs daemon logs (last 200 lines; supports -n N and -f/--follow)."
      echo ""
    ...
    case "$1" in
      ...
      update|upgrade)
        ...
        ;;
      update-warp)
        SCRIPT_WARP_UPDATE="$ZROOT_DIR/scripts/update-warp-current.sh"
        if [ ! -x "$SCRIPT_WARP_UPDATE" ]; then
          echo "Error: updater not found or not executable: $SCRIPT_WARP_UPDATE" >&2
          exit 1
        fi
        "$SCRIPT_WARP_UPDATE"
        if [ "''${2-}" = "--commit" ]; then
          if ! $GIT_BIN -C "$ZROOT_DIR" diff --quiet -- pkgs/warp-terminal-current/versions.json; then
            $GIT_BIN -C "$ZROOT_DIR" add pkgs/warp-terminal-current/versions.json
            $GIT_BIN -C "$ZROOT_DIR" commit -m "warp-current: bump to latest (vendored)"
            echo "Committed updated versions.json."
          else
            echo "No changes to commit."
          fi
        else
          echo "Note: run 'zcli stage' or 'git add/commit' to record any changes."
        fi
        ;;
      stage)
        FEATURE_REBUILD="$ZROOT_DIR/features/rebuild.sh"
      ...
      glances)
        ...
        ;;
      doom)
        FEATURE_DOOM="$ZROOT_DIR/features/doom.sh"
        if [ -f "$FEATURE_DOOM" ]; then
          # shellcheck disable=SC1090
          . "$FEATURE_DOOM"
        else
          echo "Error: feature module not found: $FEATURE_DOOM" >&2
          exit 1
        fi
        if doom_main "$@"; then :; else exit 1; fi
        ;;
      *)
