...
# Debian setup of common apps and configuration (guided by fedora-copr.sh)
...
while [[ $# -gt 0 ]]; do
  case "${1}" in
    ...
    --install-dms) INSTALL_DMS=1; shift ;;
    --dms-only)    DMS_ONLY=1; INSTALL_DMS=1; shift ;;
    --dms-src)
      DMS_SRC="${2:-}"
      if [[ -z "$DMS_SRC" ]]; then error "--dms-src requires a path"; exit 2; fi
      shift 2
      ;;
    --wezterm-only) WEZTERM_ONLY=1; shift ;;
    --fonts-only)   FONTS_ONLY=1; shift ;;
...
copy_config_files() {
  if ! ensure_nas_available; then
    if confirm "Could not access $NAS_CONFIG_DIR. Continue without copying standard config files?" y; then
      ...
      error "Aborting per user decision"; return 1
    fi
  fi
  ...
  if [[ -d "$NAS_CONFIG_DIR/yazi" ]]; then
    ...
    # After copying Yazi config, upgrade plugins if 'ya' is available
...
ensure_dms_fonts() {
  ...
  for font in "${FONTS[@]}"; do
    ...
    if fc-list | grep -qi -- "$CHECK"; then
      info "Font '$LABEL' already available"; continue
    fi
    if [[ -f "$DEST" ]]; then
      info "Font file for '$LABEL' already present at $DEST"; continue
    fi
  ...
  if (( changed )); then step "Rebuilding font cache"; run sudo fc-cache -fv; else info "No font changes needed"; fi
}
...
build_quickshell() {
  ...
  # Pass INSTALL_QMLDIR so Quickshell's QML modules are installed to the correct location
...
build_from_source_generic() {
  local src="$1"
  if [[ -z "$src" || ! -d "$src" ]]; then error "Invalid source directory: $src"; return 1; fi
...
build_breakpad() {
  ...
  # Ensure LSS is present under breakpad expected path
  ...
  # Build & install breakpad
  ( cd "$bp_dir" && run autoreconf -fi && run ./configure && run make -j"$(nproc)" )
  run sudo make -C "$bp_dir" install
  # Refresh linker and pkg-config caches
  run sudo ldconfig || true
}

ensure_breakpad() {
  # Ensure pkg-config can find 'breakpad'; try distro package first, then build from source as fallback.
...
install_dms_sources() {
  # Ensure crash reporting dependency required by Quickshell is present
  ensure_breakpad || true
  # Build Quickshell if missing or version is too old
...
ensure_matugen_min_version "2.4.1"
  # Also install the dms wrapper binary (some workflows expect it)
...
ensure_polkit_agent() {
  ...
  # Prefer GNOME agent; fallback to MATE agent.
...
EOF
  ...
  if ! pgrep -u "$USER" -x "$(basename "$agent")" >/dev/null 2>&1; then
    ...
    if (( DRY_RUN )); then
      ...
      nohup "$agent" >/dev/null 2>&1 & disown || true
    fi
  else
...
# ------------- External repo: debian.griffo.io -------------
get_codename() {
  local codename
  ...
  if [[ -z "$codename" ]]; then
    local osr="/etc/os-release"; [[ -r "$osr" ]] && . "$osr"
...
add_griffo_repo() {
  local key_url="https://debian.griffo.io/EA0F721D231FDD3A0A17B9AC7808B4DD62C41256.asc"
  local key_dst="/etc/apt/trusted.gpg.d/debian.griffo.io.gpg"
  # Prefer debian-griffo.list naming; support older debian.griffo.io.list as already-present
  local list_dst="/etc/apt/sources.list.d/debian-griffo.list"
  local alt_list="/etc/apt/sources.list.d/debian.griffo.io.list"
  local codename; codename=$(get_codename)
  # Map Kali to trixie for this third-party repo
  if is_kali; then codename="trixie"; fi

  run sudo install -d -m 0755 /etc/apt/sources.list.d /etc/apt/trusted.gpg.d

  # If either list file already exists and is non-empty, do not redo
  if [[ -s "$list_dst" || -s "$alt_list" ]]; then
    info "debian.griffo.io repo already configured"
    return 0
  fi

  step "Adding debian.griffo.io repository (codename: ${codename:-testing})"
  if (( DRY_RUN )); then
    printf "Would write key to %s and add list %s\n" "$key_dst" "$list_dst"
  else
    # Only write key if missing
    if [[ ! -s "$key_dst" ]]; then
      curl -fsSL "$key_url" | sudo gpg --dearmor --yes -o "$key_dst"
    else
      info "debian.griffo.io key already present"
    fi
    # Write the list entry; fall back to testing if codename empty
    if [[ -z "$codename" ]]; then codename="testing"; fi
    echo "deb https://debian.griffo.io/apt $codename main" | sudo tee "$list_dst" >/dev/null || true
    # If file ended up empty, retry via privileged redirection
    if [[ ! -s "$list_dst" ]]; then
      sudo sh -c "echo 'deb https://debian.griffo.io/apt $codename main' > '$list_dst'"
    fi
  fi
}
...
install_docker() {
  if is_kali; then
    warn "Docker Engine repo not available for Kali; skipping Docker install"
    return 0
  fi
  ...
  # Docker only publishes for stable codenames (e.g., bookworm). Map testing/unstable to the current stable.
  local docker_codename="$codename"
  case "$codename" in
    trixie|forky|sid|testing|unstable) docker_codename="bookworm" ;;
  esac
  run sudo apt-get update
  run sudo apt-get install -y ca-certificates curl gnupg
  if (( DRY_RUN )); then
    printf "Would ensure keyrings dir, add docker.gpg if missing, and add Docker repo for '%s' (mapped: %s)\n" "$codename" "$docker_codename"
  else
    sudo install -m 0755 -d /etc/apt/keyrings /etc/apt/sources.list.d
    # Only write key if missing to avoid overwrite prompts
    ...
    # Only create sources.list if missing
    if [[ ! -s /etc/apt/sources.list.d/docker.list ]]; then
      echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/debian $docker_codename stable" | sudo tee /etc/apt/sources.list.d/docker.list >/dev/null || true
      if [[ ! -s /etc/apt/sources.list.d/docker.list ]]; then
        sudo sh -c "echo 'deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/debian $docker_codename stable' > /etc/apt/sources.list.d/docker.list"
      fi
    else
      info "Docker APT source already present; not modifying"
    fi
  fi
  run sudo apt-get update
...
install_deb_if_exists() {
  ...
  if [[ -f "$deb_path" ]]; then
    ...
    # apt and nala both accept a path to a .deb file
...
install_nas_local_bins() {
  # Copy any executables from NAS bin dir to /usr/local/bin
...
install_apps() {
  ...
  pkg_install rustc cargo wlr-randr htop ncdu kitty alacritty 

  # Add debian.griffo.io repo and install its packages
  add_griffo_repo || true
  step "Updating package metadata (after adding griffo repo)"
  pkg_update || run sudo apt update
  step "Installing packages from griffo repo"
  pkg_install zig ghostty starship lazygit uv bun tigerbeetle || warn "Some griffo packages failed to install"
...
install_nerd_fonts() {
  ...
  for font in "${fonts[@]}"; do
    ...
    if (( DRY_RUN )); then
      ...
      curl -fL "$url" -o "$zip" || { warn "Download failed for $font"; continue; }
      run mkdir -p "$FONTS_DIR/$font"
      if unzip -q "$zip" -d "$FONTS_DIR/$font/"; then :; else
        warn "Extraction failed for $font"; rm -rf "$FONTS_DIR/$font"
      fi
      rm -f "$zip"
    fi
  done
...
main() {
  info "Starting Debian setup"

  # Choose package manager
  use_nala_if_available || try_install_nala
