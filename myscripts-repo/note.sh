...
show_usage() {
  ...
  echo -e "  ${CYAN}note.sh${NC} ${YELLOW}<text>${NC}                 - Add a new note"
  echo -e "  ${CYAN}note.sh${NC}                       - Display all notes"
  echo -e "  ${CYAN}note.sh del${NC} ${YELLOW}<number>${NC}          - Delete note by number"
  echo -e "  ${CYAN}note.sh clear${NC}                 - Clear all notes"
  echo -e "  ${CYAN}echo 'text' | note.sh${NC}        - Add note from stdin"
  echo -e "  ${CYAN}cat file | note.sh${NC}           - Add file contents as note"
  echo ""
  echo -e "${GRAY}Examples:${NC}"
  echo -e "  ${GREEN}note.sh call plumber tomorrow${NC}"
  echo -e "  ${GREEN}cat todo.txt | note.sh${NC}"
  echo -e "  ${GREEN}note.sh del 3${NC}"
}
...
display_notes() {
  if [ ! -f "$NOTES_FILE" ] || [ ! -s "$NOTES_FILE" ]; then
    ...
    echo -e "${GRAY}Use ${CYAN}note.sh <text>${GRAY} to add your first note${NC}"
    return
  fi
  ...
  case "$1" in
    ...
    "del")
      if [ -z "$2" ]; then
        ...
        echo -e "${GRAY}Usage: ${CYAN}note.sh del <number>${NC}"
        exit 1
      fi
      delete_note "$2"
      ;;
